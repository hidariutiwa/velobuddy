import { AddFavoriteRequest, FavoritePlace, PlaceCache, UpdateFavoriteRequest } from "@/types/place";
...
describe("getFavoritesByUserId", () => {
  ...
  it("orderBy: { createdAt: 'desc' } で Prisma を呼ぶ", async () => {
    ...
    const callArg = mockPrismaUserFavoritePlace.findMany.mock.calls[0][0] as {
      where: { userId: number };
...
describe("deleteFavorite", () => {
  it("where: { id, userId } で deleteMany を呼ぶ", async () => {
    mockPrismaUserFavoritePlace.deleteMany.mockResolvedValueOnce({ count: 1 });

    await deleteFavorite(10, 1);

    expect(mockPrismaUserFavoritePlace.deleteMany).toHaveBeenCalledTimes(1);
    const callArg = mockPrismaUserFavoritePlace.deleteMany.mock.calls[0][0] as {
