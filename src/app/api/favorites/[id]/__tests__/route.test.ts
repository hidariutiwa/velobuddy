...
import { deleteFavorite, getFavoriteById, updateFavorite } from "@/lib/db/place";
...
const mockUpdateFavorite = updateFavorite as jest.MockedFunction<typeof updateFavorite>;
const mockDeleteFavorite = deleteFavorite as jest.MockedFunction<typeof deleteFavorite>;
const mockGetFavoriteById = getFavoriteById as jest.MockedFunction<typeof getFavoriteById>;
...
describe("PATCH /api/favorites/[id]", () => {
  it("visited を更新した FavoritePlace を 200 で返す", async () => {
    mockGetServerSession.mockResolvedValueOnce(mockSession as Parameters<typeof mockGetServerSession>[0] extends undefined ? never : Awaited<ReturnType<typeof mockGetServerSession>>);
    mockGetFavoriteById.mockResolvedValueOnce({ id: 10, userId: 1 } as Awaited<ReturnType<typeof getFavoriteById>>);
    mockUpdateFavorite.mockResolvedValueOnce({ ...mockFavoritePlace, visited: true });
  ...
  it("memo を null に更新できる", async () => {
    mockGetServerSession.mockResolvedValueOnce(mockSession as Parameters<typeof mockGetServerSession>[0] extends undefined ? never : Awaited<ReturnType<typeof mockGetServerSession>>);
    mockGetFavoriteById.mockResolvedValueOnce({ id: 10, userId: 1 } as Awaited<ReturnType<typeof getFavoriteById>>);
    mockUpdateFavorite.mockResolvedValueOnce({ ...mockFavoritePlace, memo: null });
