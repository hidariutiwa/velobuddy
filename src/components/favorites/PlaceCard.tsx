...
export function PlaceCard({
  ...
  const handleMemoBlur = () => {
    onMemoUpdate(favorite.id, memoValue.trim() !== "" ? memoValue.trim() : null);
  };
  ...
  return (
    <div className="flex w-full flex-col gap-3 rounded-xl bg-white p-4 shadow">
      <div className="flex w-full items-start gap-4">
        ...
        <div className="flex min-w-0 grow flex-col gap-0.5">
          ...
          {favorite.place.address && (
            <p className="text-sm text-zinc-500">{favorite.place.address}</p>
          )}
        </div>
        <button
          type="button"
      ...
      <div className="flex items-center gap-3">
        <button
          type="button"
          onClick={() => onVisitedToggle(favorite.id, !favorite.visited)}
